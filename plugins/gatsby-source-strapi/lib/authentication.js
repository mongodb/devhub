...
module.exports = async ({ loginData, reporter, apiURL }) => {
    const validIndentifier =
        (0, _fp.has)('identifier', loginData) &&
        loginData.identifier.length !== 0;
    const validPassword =
        (0, _fp.has)('password', loginData) && loginData.password.length !== 0;
    ...
    if (validIndentifier && validPassword) {
        ...
        try {
            const loginResponse = await _axios.default.post(
                `${apiURL}/auth/local`,
                loginData
            );
