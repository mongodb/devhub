...
const Card = ({ card, guidesMetadata }) => {
    const getCardTitle = cardSlug =>
        getNestedValue([cardSlug, 'title'], guidesMetadata);
    const getCompletionTime = cardSlug =>
        getNestedValue([cardSlug, 'completionTime'], guidesMetadata) ||
        DEFAULT_COMPLETION_TIME;
    const getPills = cardSlug =>
        getNestedValue([cardSlug, 'languages'], guidesMetadata);

    const cardContent = () => {
        const cardSlug = getNestedValue(['argument', 0, 'value'], card);
        const languagesNode =
            card.name !== 'multi-card' ? getPills(cardSlug) : undefined;
        const multiCardEntries = getNestedValue(
            ['children', 0, 'children'],
            card
        );
        const innerContent = (
            <React.Fragment>
                <div className="guide__title">
                    {card.name === 'card' ? getCardTitle(cardSlug) : cardSlug}
                </div>
                ...
                {card.name !== 'multi-card' && languagesNode && (
                    <CardPills
                        pillsNode={languagesNode}
                        pillsetName="drivers"
                    />
                )}
                {card.name === 'card' && (
                    <div className="guide__time">
                        {getCompletionTime(cardSlug)}min
                    </div>
                )}
            </React.Fragment>
        );
        ...
        return (
            <Link
                to={getNestedValue(['argument', 0, 'value'], card)}
