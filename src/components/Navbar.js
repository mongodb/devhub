...
export default class Navbar extends Component {
    ...
    checkForLink = location => {
        if (
            location.hostname === DOCS_SITE ||
            location.hostname === STAGING_SITE
        ) {
        ...
        if (this.isLocalhost(location.hostname)) {
            ...
            return link !== null
                ? link
                : this.checkUrlItems(process.env.GATSBY_SITE, URL_SUBDOMAINS);
        }
    ...
    checkUrlItems = (slug, urlItems) => {
        const urlMapping = Object.entries(urlItems).find(([, value]) =>
            value.includes(slug)
        );
    ...
    modifyActiveLink = () => {
        return `{"links": [
                    ...
                    {"url": "https://docs.mongodb.com/cloud/","text": "Cloud", "active": ${this.isActiveLink(
                        'Cloud'
                    )}},
                    {"url": "https://docs.mongodb.com/tools/","text": "Tools", "active": ${this.isActiveLink(
                        'Tools'
                    )}},
