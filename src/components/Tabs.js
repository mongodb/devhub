...
export default class Tabs extends Component {
    constructor(props) {
        ...
        const tabsetName =
            getNestedValue(['options', 'tabset'], nodeData) ||
            this.generateAnonymousTabsetName(nodeData);
    ...
    componentDidUpdate(prevProps) {
        ...
        if (
            ...
            JSON.stringify(this.context.activeTabs) !==
                JSON.stringify(this.state.activeTabs) // eslint-disable-line react/destructuring-assignment
        ) {
    ...
    sortTabset = (nodeData, referenceArray) => {
        return nodeData.children.sort((a, b) => {
            ...
            return (
                referenceArray.indexOf(aValue) - referenceArray.indexOf(bValue)
            );
        });
    };
    ...
    render() {
        ...
        const tabs =
            tabsetName === 'platforms' ||
            PLATFORMS.some(p => tabsetName.includes(p))
        ...
        return (
            <React.Fragment>
                {isHeaderTabset || isHidden || (
                    <ul
                        className="tab-strip tab-strip--singleton"
                        role="tablist"
                    >
                        {tabs.map((tab, index) => {
                            const tabId = getNestedValue(
                                ['options', 'tabid'],
                                tab
                            );
                            const tabTitle =
                ...
                {tabs.map((tab, index) => {
                    ...
                    const tabContent =
                        !activeTabs ||
                        Object.getOwnPropertyNames(activeTabs).length === 0
