...
export default class TabsPillstrip extends Component {
    ...
    render() {
        ...
        const pillNodes = getNestedValue(
            [this.pillstripName, 'children'],
            pillstrips
        );
        const pills = pillNodes
            ? pillNodes.map(pill => getNestedValue(['options', 'tabid'], pill))
            : [];
        return (
            <Pills
                pills={pills}
                pillsetName={this.pillstripName}
                dataTabPreference={this.pillstripName}
            />
        );
    }
}
