...
const ListTable = ({ nodeData, nodeData: { children }, ...rest }) => {
    ...
    const stubColumnCount =
        parseInt(getNestedValue(['options', 'stub-columns'], nodeData), 10) ||
        0;
    const headerRows = children[0].children[0].children.slice(
        0,
        headerRowCount
    );
    const bodyRows = children[0].children.slice(headerRowCount);
    const customWidth =
        getNestedValue(['options', 'width'], nodeData) || 'auto';
...
const ListTableBodyRow = ({ row, rowIndex, stubColumnCount, ...rest }) => (
    <tr className={rowIndex % 2 === 0 ? 'row-odd' : 'row-even'}>
        {row.map((column, colIndex) => (
            <td
                className={`${colIndex <= stubColumnCount - 1 ? 'stub' : ''}`}
                key={colIndex}
            >
...
ListTable.propTypes = {
    nodeData: PropTypes.shape({
        ...
        options: PropTypes.shape({
            ...
            'header-rows': PropTypes.oneOfType([
                PropTypes.number,
                PropTypes.string,
            ]),
            'stub-columns': PropTypes.oneOfType([
                PropTypes.number,
                PropTypes.string,
            ]),
