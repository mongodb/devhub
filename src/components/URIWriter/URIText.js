...
function optionStringifier(options, uri) {
    ...
    options.forEach(({ name, type, value }) => {
        ...
        if (!paramValue) {
            paramValue =
                type === 'pass-through' ? value : uri[name] || `$[${name}]`;
        }
...
function generateURI(uri, activeDeployment, templateType) {
    ...
    if (
        Object.entries(uri).length === 0 &&
        activeDeployment === TEMPLATE_TYPE_ATLAS
    ) {
        template = TEMPLATE_TYPE_ATLAS_36;
    } else if (
        Object.entries(uri).length === 0 &&
        activeDeployment === TEMPLATE_TYPE_SELF_MANAGED
    ) {
    ...
    const hostlist =
        uri.hostlist && uri.hostlist.length > 0 ? uri.hostlist : '$[hostlist]';
...
function replacePlaceholderWithURI(value, activeDeployment, uri) {
    return value
        .replace(
            URI_PLACEHOLDER,
            generateURI(uri, activeDeployment, 'template')
        )
        .replace(USERNAME_PLACEHOLDER, uri.username || '$[username]')
        .replace(
            URISTRING_SHELL_PLACEHOLDER,
            generateURI(uri, activeDeployment, 'templateShell')
        )
        .replace(
            URISTRING_SHELL_NOUSER_PLACEHOLDER,
            generateURI(uri, activeDeployment, 'templatePasswordRedactedShell')
        );
}
