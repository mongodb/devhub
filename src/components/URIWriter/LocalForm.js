...
export default class LocalForm extends Component {
    ...
    updateHostlist = () => {
        ...
        this.setState(
            prevState => ({
                uri: {
                    ...prevState.uri,
                    hostlist: Object.values(hostInputs).filter(
                        host => !this.hostnameHasError(host) && host !== ''
                    ),
                },
            }),
            () => handleUpdateURIWriter(this.state.uri) // eslint-disable-line react/destructuring-assignment
    ...
    updateHostInputs = (name, value) => {
        ...
        if (
            value === '' &&
            Object.entries(hostInputs).length > 1 &&
            Object.values(hostInputs).includes('')
        ) {
        ...
        this.setState({ hostInputs: updatedHostInputs }, () =>
            this.updateHostlist()
        );
    };
    ...
    render() {
        ...
        return (
            <React.Fragment>
                <div
                    className="mongodb-form__prompt"
                    style={{ display: 'block' }}
                >
                    <div className="mongodb-form__label">
                        Server deployment type
                    </div>
                    <ul className="guide__pills">
                        {LOCAL_ENVS.map((env, index) => (
                            <li
                                className={`uriwriter__toggle guide__pill ${localEnv ===
                                    env.key && 'guide__pill--active'}`}
                                key={index}
                            >
                                <span
