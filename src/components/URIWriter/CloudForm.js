...
export default class CloudForm extends Component {
    ...
    parseShell = atlasString => {
        ...
        const [envAndClusters, error] = this.parseOutEnvAndClusters(
            splitOnSpaceClusterEnv
        );
        if (error) {
            this.clearURI();
            return false;
    ...
    parseAtlasString = (atlasString, atlasVersion) => {
        const { handleUpdateURIWriter } = this.props;
        const matchesArray =
            atlasString.match(re3dot4) || atlasString.match(re3dot6);
        if (!matchesArray) {
            this.clearURI();
            return;
        }
    ...
    parseConnectionString = (pastedValue, target = undefined) => {
        ...
        if (target) {
            const error = target.setCustomValidity(
                this.formHasError(atlasString)
            );
            if (error) {
                this.clearURI();
            }
    ...
    render() {
        ...
        return (
            <label className="mongodb-form__prompt">
                <span className="mongodb-form__label">
