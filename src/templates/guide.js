...
export default class Guide extends Component {
    constructor(propsFromServer) {
        ...
        this.bodySections = this.sections.filter(section =>
            Object.keys(SECTION_NAME_MAPPING).includes(section.name)
        );
    ...
    recalculate = () => {
        ...
        if (isScrollable) {
            ...
            const headings = this.sectionRefs.map((ref, index) => [
                ref,
                this.bodySections[index].name,
            ]);
            ...
            currentPosition =
                (scrollTop + currentPosition * 0.8 * window.innerHeight) /
                height;
    ...
    addGuidesTabset = (tabsetName, tabData) => {
        const tabs = tabData.map(tab =>
            getNestedValue(['options', 'tabid'], tab)
        );
        if (tabsetName === 'cloud') {
    ...
    matchArraySorting = (tabs, referenceArray) =>
        referenceArray.filter(t => tabs.includes(t));
        ...
        this.setState(
            prevState => ({
                [tabsetName]: this.matchArraySorting(
                    Array.from(
                        new Set([...(prevState[tabsetName] || []), ...tabs])
                    ),
                    constants
                ),
            }),
            ...
            return this.sections.map(section => {
                return (
                    <ComponentFactory
                        nodeData={section}
                        refDocMapping={
                            getNestedValue(['__refDocMapping'], pageContext) ||
                            {}
                        }
                    />
                );
            ...
            <React.Fragment>
                ...
                <div className="content">
                    <TOC
                        ...
                        sectionKeys={this.bodySections.map(
                    ...
                    <div id="main-column" className="main-column">
                        <div className="body" data-pagename={pageSlug}>
                            <GuideBreadcrumbs />
                            <GuideHeading
                                author={findKeyValuePair(
                                    this.sections,
                                    'name',
                                    'author'
                                )}
                                cloud={cloud}
                                description={findKeyValuePair(
                                    this.sections,
                                    'name',
                                    'result_description'
                                )}
                                ...
                                time={findKeyValuePair(
                                    this.sections,
                                    'name',
                                    'time'
                                )}
                                title={findKeyValuePair(
                                    this.sections,
                                    'type',
                                    'heading'
                                )}
                    ...
                    {!process.env.PREVIEW_PAGE && (
                        <Widgets
                            guideName={pageSlug}
                            snootyStitchId={pageContext.snootyStitchId}
                        />
                    )}
                </div>
            </React.Fragment>
