...
const mountedToc = mockData =>
    mount(<TableOfContents toctreeData={mockData} />);
const shallowToc = mockData =>
    shallow(<TableOfContents toctreeData={mockData} />);
...
describe('Table of Contents testing', () => {
    describe('Table of Contents unit tests', () => {
        ...
        it('TOC exists with proper number of sections and nodes per section', () => {
            ...
            expect(testComponent.find('.toctree-l2')).toHaveLength(
                numDriverNodes
            );
        });
        ...
        describe('TOC navigation should render and work as expected', () => {
            it('TOC slugs work as expected', () => {
                ...
                const testTOCLink = testComponent
                    .find('ul li.toctree-l1 .reference')
                    .first();
            ...
            it('TOC external navigation (urls) should work as expected', () => {
                ...
                expect(biConnectorLink.prop('href')).toBe(
        ...
        describe('TOC options work as expected', () => {
            it('TOC supports a drawer option', () => {
                ...
                const numUseCasesNodes =
                    mockTocData.children[3].children.length;
                expect(testComponent.find('.toctree-l2')).toHaveLength(
                    numUseCasesNodes
                );
                expect(window.location.pathname).toBe('/');
                ...
                const numPlatformNodes =
                    mockTocData.children[2].children.length;
                expect(testComponent.find('.toctree-l2')).toHaveLength(
                    numPlatformNodes
                );
            });
