...
describe('URIWriter', () => {
    describe('LocalForm', () => {
        ...
        it('does not show an error', () => {
            expect(
                wrapper.find('.mongodb-form__status--invalid').exists()
            ).toEqual(false);
        });
      ...
      expect(state.hostInputs).toHaveLength(2);
        ...
        it('has local MongoDB selected as the active pill', () => {
            expect(wrapper.find('.guide__pill--active').text()).toEqual(
                'Local MongoDB'
            );
        });
        ...
        describe('when replica set is selected', () => {
            beforeAll(() => {
                const replicaSetPill = wrapper.find(
        ...
        describe('when updated with invalid host inputs', () => {
            beforeAll(() => {
                wrapper = mount(
                    <LocalForm handleUpdateURIWriter={mockCallback} />
                );
            });
            ...
            it('shows an error', () => {
                expect(
                    wrapper.find('.mongodb-form__status--invalid').length
                ).toBe(1);
            });
        });
    });
    ...
    describe('with an Atlas cluster', () => {
        ...
        describe('when a MongoDB 3.4 string is entered', () => {
            ...
            it('parses the string correctly', () => {
                ...
                const atlasTextarea = wrapper.find(
                    "textarea[name='connectionString']"
                );
        ...
        describe('when a MongoDB 3.6 string is entered', () => {
            ...
            it('parses the string correctly', () => {
                const inputString =
                    'mongodb+srv://myUsername2:myPassword2@cluster0-juau5.mongodb.net/myDatabaseName2?';
                const atlasTextarea = wrapper.find(
                    "textarea[name='connectionString']"
                );
                atlasTextarea.instance().value = inputString;
