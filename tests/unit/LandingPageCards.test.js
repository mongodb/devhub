...
const shallowLandingPageCards = ({ guides, guidesMetadata }) =>
    shallow(
        <LandingPageCards guides={guides} guidesMetadata={guidesMetadata} />
    );
...
describe('LandingPageCards component', () => {
    describe('when mounted with one type of guide', () => {
        ...
        beforeAll(() => {
            wrapper = mountLandingPageCards({
                guides: mockGuides,
                guidesMetadata: mockGuidesMetadata,
            });
            shallowWrapper = shallowLandingPageCards({
                guides: mockGuides,
                guidesMetadata: mockGuidesMetadata,
            });
        });
        ...
        it('shows the correct icon', () => {
            expect(
                wrapper.find('.guide-category__title--getting-started')
            ).toHaveLength(1);
            expect(
                wrapper.find('.guide-category__title--use-case')
            ).toHaveLength(0);
            expect(
                wrapper.find('.guide-category__title--deep-dive')
            ).toHaveLength(0);
        });
    ...
    describe('when mounted with two types of guides', () => {
        ...
        beforeAll(() => {
            wrapper = mountLandingPageCards({
                guides: mockGuides,
                guidesMetadata: mockGuidesMetadataMultiple,
            });
            shallowWrapper = shallowLandingPageCards({
                guides: mockGuides,
                guidesMetadata: mockGuidesMetadataMultiple,
