...
const mountTabs = ({
    ...
    return mount(
        <TabContext.Provider
            value={{ activeTabs, setActiveTab: mockSetActiveTab }}
        >
...
const shallowTabs = ({ mockData, mockAddTabset }) =>
    shallow(<Tabs nodeData={mockData} addTabset={mockAddTabset} />, {
        context,
    });
...
describe('Tabs testing', () => {
    describe('Tab unit tests', () => {
        ...
        it('active tab is set in DOM', () => {
            expect(
                wrapper
                    .find('.tab-strip__element[aria-selected="true"]')
                    .exists()
            ).toEqual(true);
        });
        ...
        it('exists non-active tab', () => {
            expect(
                wrapper
                    .find('.tab-strip__element[aria-selected="false"]')
                    .exists()
            ).toEqual(true);
        });

        it('clicking new non-active tab calls function', () => {
            const nonactiveTab = wrapper
                .find('.tab-strip__element[aria-selected="false"]')
                .first();
